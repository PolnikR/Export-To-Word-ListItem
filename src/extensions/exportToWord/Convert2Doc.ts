import { SPListOperations, SPCore, SPCommonOperations, ISPBaseResponse, SPFieldOperations, IFields } from 'spfxhelper';
import { Log } from '@microsoft/sp-core-library';
//import { IListItemsResponse, IListView } from 'spfxhelper/dist/SPFxHelper/Props/ISPListProps'; 
import { IListItemsResponse, IListView }from 'spfxhelper/dist/SPFxHelper/Props/ISPListProps';
import { SPHttpClient } from '@microsoft/sp-http';
//import ExportToWordCommandSet from './ExportToWordCommandSet';
...
class Convert2Doc {
    ...
    //private currentViewFields:{[key:string]:string}
    ...
    */
    ...
    public async createDocument(): Promise<void> {
        ...
        const items: IListItemsResponse = await this.getItems();


        if (await this.validateColumnTypes()) {

            const showQnA: boolean = confirm("QnA format can be printed with the selected view. Do you want to proceed with the QnA format ?\n Press Ok to continue with QnA format, cancel to continue with List Format");
